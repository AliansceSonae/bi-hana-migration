...
LEFT OUTER JOIN `also-analytics-model-nonprod.3_MATERIALIZADO_S4.DIM_EDIFICIO`
    AS EDIFICIO
    ON  EDIFICIO.BUKRS = VIBDRO.BUKRS
    AND EDIFICIO.SWENR = VIBDRO.SWENR
    AND EDIFICIO.SGENR = VIBDRO.SGENR
LEFT OUTER JOIN `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_OBJETO_LOCACAO_MEDICAO_MENSAL_4B` ()
    AS MEDICAO
