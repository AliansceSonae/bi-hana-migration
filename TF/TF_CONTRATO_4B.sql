CREATE OR REPLACE TABLE FUNCTION `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_CONTRATO_4B` ()
AS
...
FROM
    `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_ATVABRASCE_4B`()
GROUP BY
    ATA_DSATIVIDADE,
    ATA_CDSHOPPING
),
...
FROM
    (
        ...
        FROM
            `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_UNIDADE_HIST_4B`()
        WHERE
...
SELECT
    UNIDADE.*
FROM
    `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_UNIDADE_HIST_4B`() AS UNIDADE
...
FROM
    `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_UNIDADE_ECONOMICA_4B` () AS UE
...
SELECT
    Q1.*
FROM
    `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_GRPABRASCE_4B`() AS Q1
    INNER JOIN (
        SELECT
            SHP_CDSHOPPING,
            ZGRABRASCE,
        ...
        FROM
            `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_GRPABRASCE_4B`()
        GROUP BY
            SHP_CDSHOPPING,
            ZGRABRASCE
...
FROM
    ...
    LEFT OUTER JOIN `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_GRPABL_4B` () AS GRPABL 
    ...
    LEFT OUTER JOIN `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_OBJETO_LOCACAO_4B` () AS OBJLOC 
    ...
    LEFT OUTER JOIN `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_GRPABRASCE_4B`()AS GRPABRASCE_DEFAULT ON GRPABRASCE_DEFAULT.KEY_GRPABRASCE = 'S4' || '-' || OBJLOC.CSHP_CDSHOPPING || '-' || '99'
...
FROM
    (
        SELECT
            ...
            CSHP_DTCADASTRO
        FROM
            `also-analytics-model-nonprod.1_AQUISICAO_S4.dm_contratoshp_cshp` AS CONTR_MXM
    )
)
SELECT
    ...
    DATE_DIFF(DATE(SAIDA.RECNBEG),DATE(SAIDA.RECNEND1ST), MONTH) + 1 AS CSHP_PRAZO,
    ...
    CASE
        ...
        ELSE SAIDA.RECNNR
    END AS CC_SMIVE,
    DATE_DIFF(DATE(SAIDA.RECNENDABS), DATE(SAIDA.RECNBEG), DAY) AS CC_DIAS_VENCTO,
    SAIDA.FLAG_CTO AS FLAG_CTO,
    CASE
        WHEN DATE(SAIDA.RECNBEG) > CURRENT_DATE() THEN 'N'
        WHEN DATE(SAIDA.RECNENDABS) < CURRENT_DATE() THEN 'N'
        ELSE 'S'
    END AS FLAG_ATIVO
FROM
    var_saida AS SAIDA
    LEFT OUTER JOIN `also-analytics-model-nonprod.1_AQUISICAO_S4.dim_contrato_sb` AS CONTR_ECC ON CONTR_ECC.BUKRS = SAIDA.CSHP_CDSHOPPING
