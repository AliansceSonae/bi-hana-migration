CREATE OR REPLACE TABLE FUNCTION `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_VENDA_DIARIA_4B`() 
AS
...
var_venda_informada AS (
		...
		FROM `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_VENDA_INTRAMALL_4B`() AS VENDA
		LEFT OUTER JOIN var_venda_informada_attr AS ATTR
			...
			AND FORMAT_DATE('%Y%m', ATTR.CALMONTH)  = VENDA.CALMONTH
		GROUP BY
			VENDA.CSHP_CDSHOPPING,
			VENDA.BUKRS,
			VENDA.INTRENO_VICNCN,
...
var_venda_auditada AS (
		...
		FROM `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_VENDA_AUDITADA_4B` () AS VENDA
...
var_maior_venda AS (
			SELECT
				...
				GREATEST(SUM(NET_SALES_AUDITADA)  , SUM(NET_SALES_INTRAMALL))		AS NET_SALES,
				GREATEST(SUM(GROSS_SALES_AUDITADA), SUM(GROSS_SALES_INTRAMALL))		AS GROSS_SALES,
				GREATEST(SUM(CC_VOLVED_AUDITADA)  , SUM(CC_VOLVED_INTRAMALL))		AS CC_VOLVED
		FROM (
			SELECT
				VENDA.CSHP_CDSHOPPING,
				VENDA.BUKRS,
				...
				VENDA.CSHP_CDCONTRATO_MXM,
				VENDA.CC_CDCONTRATO,
				PARSE_DATE('%Y%m', VENDA.CALMONTH) AS CALMONTH,
				VENDA.RPDATE,
				VENDA.SALESTYPE,
				VENDA.XMETXT,
				VENDA.SMENR,
    ...
    SELECT
        ...
        CC_VOLVED,
        CC_FQMFLART_SONAE,
        CASE WHEN CC_FQMFLART_SONAE = 0	THEN 0 ELSE CC_VOLVED END AS CC_VOLVED_COM_FQMFLART,
        CASE WHEN CC_VOLVED = 0			THEN 0 ELSE FQMFLART  END AS CC_FQMFLART_COM_VOLVED,
        NET_SALES,
        GROSS_SALES
