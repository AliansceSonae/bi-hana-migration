CREATE OR REPLACE TABLE FUNCTION aliansceSonae.2_business.financerio.s4::TF_CONTRATO_CONDICAO_MENSAL_4B
       
AS 
BEGIN
	DECLARE var_calmonth_limit STRING(6);
	
	var_calmonth_limit = TO_STRING(ADD_YEARS(CURRENT_TIMESTAMP(), 10), 'YYYYMM');
...
WITH var_condicao_tempo AS (
    SELECT 
        ...
        COND.CALCRULEPARA2
    FROM aliansceSonae.4_presentation.financeiro.s4::CVW_CONTRATO_CONDICAO_4P AS COND
    LEFT OUTER JOIN TARGET::DIM_TEMPO_MENSAL AS TEMPO
        ON  TO_STRING(COND.CC_VALIDFROM, 'YYYYMM') <= TEMPO.CALMONTH
        AND TO_STRING(COND.CC_VALIDTO  , 'YYYYMM') >= TEMPO.CALMONTH
    WHERE TEMPO.CALMONTH <= :var_calmonth_limit
...
--				THEN TO_DECIMAL(REPLACE(MAP(COND.CALCRULEPARA2,'','0',COND.CALCRULEPARA2), ',', '.')) 
            THEN REPLACE(MAP(COND.CALCRULEPARA2,'','0',COND.CALCRULEPARA2), ',', '.')
...
var_saida1 AS ( 
    ...
    LEFT OUTER JOIN also-analytics-model-nonprod.DIM_UNIDADE_HIST_4P AS UNIDADE
    ...
    FROM (
        SELECT
            ...
            CASE WHEN COND.CALCRULE = 'E1' 
                THEN S1.UNITPRICE_Z010 * (COND.CALCRULEPARA2 / 100) 
                ELSE 0
