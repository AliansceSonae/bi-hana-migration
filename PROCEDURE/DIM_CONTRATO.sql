CREATE OR REPLACE PROCEDURE `also-analytics-model-nonprod.3_MATERIALIZADO_S4.DIM_CONTRATO` () 
OPTIONS(strict_mode=false)
BEGIN
		DELETE FROM `also-analytics-model-nonprod.3_MATERIALIZADO_S4.DIM_CONTRATO` WHERE 1=1;

    CREATE OR REPLACE TABLE `also-analytics-model-nonprod.3_MATERIALIZADO_S4.DIM_CONTRATO`
    AS
    SELECT *, CURRENT_TIMESTAMP() AS CHANGE_TIME FROM `also-analytics-model-nonprod.2_NEGOCIO_S4.TF_CONTRATO_4B`();

		-- INSERT INTO `also-analytics-model-prod.1_AQUISICAO_S4.DIM_CONTRATO`
		-- (INTRENO_VICNCN, INTRENO_VIBDRO, INTRENO_VIBDBU, BUKRS, RECNNR, OBJNR, IMKEY, RECNTYPE, RECNTYPE_TEXT15, RECNTYPE_TEXT30, RECNBEG, 
		...
		-- FROM `also-analytics-model-prod.2_NEGOCIO_S4.TF_CONTRATO`();
END;
